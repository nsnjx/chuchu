...
import 'model/zapRecordsDB_isar.dart';
import 'model/zapsDB_isar.dart';
...
class Zaps {
    ...
    zapsSubscription = Connect.sharedInstance.addSubscriptions(subscriptions, closeSubscription: false,
      ...
      switch (event.kind) {
        ...
        case 9735:
          ...
          break;
        default:
          LogUtils.v(() => 'feed unhandled message ${event.toJson()}');
          break;
      }
  ...
  static Future<ZapsDBISAR?> getZapsInfoFromDB(String lnurl) async {
    ...
    return await isar.zapsDBISARs.where().lnURLEqualTo(lnurl).findFirst();
  }
  ...
  static Future<List<ZapRecordsDBISAR?>> searchZapRecordsFromDB(
    ...
    List<ZapRecordsDBISAR?> maps = [];
    if (bolt11 != null) {
      maps = await queryBuilder.bolt11EqualTo(bolt11).sortByPaidAtDesc().findAll(limit: limit);
    }
    if (sender != null) {
      maps = await queryBuilder.senderEqualTo(sender).sortByPaidAtDesc().findAll(limit: limit);
    }
    if (recipient != null) {
      maps =
          await queryBuilder.recipientEqualTo(recipient).sortByPaidAtDesc().findAll(limit: limit);
    }
  ...
  static Future<ZapRecordsDBISAR> handleZapRecordEvent(Event event) async {
    ...
    Feed.sharedInstance.handleZapNotification(zapRecordsDB, event);
    return zapRecordsDB;
  }
